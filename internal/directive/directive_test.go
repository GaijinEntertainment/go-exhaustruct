...
func Test_Parse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			d, found := directive.Parse(tt.text)
			assert.Equal(t, tt.found, found, "found mismatch")
			assert.Equal(t, tt.directive, d, "directive mismatch")
		})
	}
}
...
`,
			line: 3,
			want: directive.Optional, // first wins
		},
		{
...
func Test_NewFile_Testdata(t *testing.T) {
	...
	expectDirective := map[int]directive.Directive{
		// Doc comments
